...
# 定义train函数
def train(tokenizer, model, start_epoch, epochs, device='cuda'):
    ...
    # 绘制loss变化曲线
    plt.figure()
    plt.plot(range(start_epoch, epochs), loss_all, label='Training Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    ...
    # 绘制BLEU4分数变化曲线
    plt.figure()
    plt.plot(range(start_epoch, epochs), bleu4_scores, label='Validation BLEU4 Score')
    plt.xlabel('Epoch')
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('model/transformer_epoch_73_bleu4_21.54.pt'))
    model = model.to(device)

    # 继续训练
    train(tokenizer, model, start_epoch=74, epochs=100, device=device)
